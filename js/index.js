$(document).ready(function () {
	...
	function load_handler() {
		...
		let random = 4;
		var obj = "https://cdn.omnimkt.com/hkjc/";
	...
	function pageInit() {
		...
		$(".loading .go").on("click", loading_btn);
		...
		$(".p5 .back").on("click", p5_back_btn);

		// $(".p2 .p2_1 .look_video").on("click", look_video_btn);
		$(".p2 .p2_1 .look_video").on("click", play_video1_btn);
		$(".p4 .p4_7 .look_video").on("click", play_video2_btn);
	...
	function menu2_btn() {
		...
		$(".p3").show();
		downUpSwiper();
		down_up_swiper.slideTo(0, 300, false);
		down_up_swiper.update();
	}
	...
	function menu3_btn() {
		...
		$(".p4").show();
		wealSwiper();
		weal_swiper.slideTo(0, 300, false);
		weal_swiper.update();
	}
		...
		down_up_swiper = new Swiper('.p3 .swiper-container', {
			...
			on: {
				slideChange: function () {
					...
					index == 5 ? $(".p3 .down_icon").hide() : $(".p3 .down_icon").show()
				}
			}
		});
	}
		...
		down_up_swiper2 = new Swiper('.p5 .swiper-container', {
			...
			on: {
				slideChange: function () {
					...
					var index = this.realIndex;
					console.log('index: ', index);
					// index == 9 ? $(".p5 .down").hide() : $(".p5 .down").show()
					if (index == 11) {
						$(".p5").hide();
						$(".p2").show();
						prevNextSwiper();
	...
	function p5_down_btn() {
		down_up_swiper2.slideNext();
	}
		...
		weal_swiper = new Swiper('.p4 .swiper-container', {
			...
			on: {
				slideChange: function () {
					// 获取当前活动下标
					var index = this.realIndex;
					index == 7 ? $(".p4 .down_icon").hide() : $(".p4 .down_icon").show()
				}
			}
		});
	}
	...
	function monitor_handler() {
		imonitor.add({ obj: $(".p1 .menu .menu_img1"), action: 'touchstart', category: 'default', label: '香港赛马会历史' });
		imonitor.add({ obj: $(".p1 .menu .menu_img2"), action: 'touchstart', category: 'default', label: '会员活动' });
		imonitor.add({ obj: $(".p1 .menu .menu_img3"), action: 'touchstart', category: 'default', label: '会员福利' });
		imonitor.add({ obj: $(".p1 .menu .menu_img4"), action: 'touchstart', category: 'default', label: '北京会所全景照片' });
		imonitor.add({ obj: $(".loading .go"), action: 'touchstart', category: 'default', label: '点击进入主页' });
		imonitor.add({ obj: $("a.bgmPlay"), action: 'touchstart', category: 'default', label: '音乐播放' });
		imonitor.add({ obj: $(".p2 .swiper-slide .p2_1 .look_video"), action: 'touchstart', category: 'default', label: '查看视频' });
	} //end func
}); //end ready
