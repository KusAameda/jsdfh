...
var NodeContainer = function () {
    function NodeContainer(node, parent, resourceLoader, index) {
        ...
        if(node.id == 'zyq'){
            ...
            console.log(style.webkitTextStrokeWidth);
            console.log(style.webkitTextStrokeColor);
        }
...
var CanvasRenderer = function () {
    ...
    _createClass(CanvasRenderer, [{
        ...
        key: 'renderTextNodeByStroke',
        value : function renderTextNodeByStroke(textBounds, color, font, textDecoration, textShadows, textStrokeWidth, textStrokeColor){
            var _this4 = this;

            this.ctx.font = [font.fontStyle, font.fontVariant, font.fontWeight, font.fontSize, font.fontFamily].join(' ');

            console.log("textDecoration:" + textShadows);
            textBounds.forEach(function (text) {
                ...
                if (textShadows && text.text.trim().length) {
                    textShadows.slice(0).reverse().forEach(function (textShadow) {
                        ...
                        //先渲染text，如果有阴影，同时渲染阴影，然后将阴影效果去掉；如果有描边，渲染描边效果
                        _this4.ctx.shadowOffsetX = 0;
                        _this4.ctx.shadowOffsetY = 0;
                        _this4.ctx.shadowBlur = 0;
                        _this4.ctx.lineWidth = textStrokeWidth;
                        _this4.ctx.strokeStyle = textStrokeColor;
                        _this4.ctx.strokeText(text.text, text.bounds.left, text.bounds.top + text.bounds.height);

                        
                    });
                } else {        
                    _this4.ctx.lineWidth = parseInt(textStrokeWidth);
                    _this4.ctx.strokeStyle = textStrokeColor;
                    _this4.ctx.strokeText(text.text, text.bounds.left, text.bounds.top + text.bounds.height);
                    _this4.ctx.fillText(text.text, text.bounds.left, text.bounds.top + text.bounds.height);    
                }
                    ...
                    container.childNodes.forEach(function (child) {
                        if (child instanceof _TextContainer2.default) {
                            ...
                            if(style.strokeWidth && style.strokeWidth!='0px'){
                                _this.target.renderTextNodeByStroke(child.bounds, style.color, style.font, style.textDecoration, style.textShadow, style.strokeWidth, style.strokeColor);
                            }else{
