...
function importShare() {
			...
			wx.ready(function() {
				...
				wx.hideMenuItems({
					menuList: ['menuItem:share:appMessage', 'menuItem:share:timeline',
						'menuItem:share:qq', 'menuItem:share:weiboApp', 'menuItem:copyUrl',
						'menuItem:openWithQQBrowser', 'menuItem:openWithSafari',
						'menuItem:share:QZone', 'menuItem:share:email'
	...
	share.wxShare = function() {
		if (share.wxSigned) {
			...
			if (localStorage.openid) {
				sharelink = sharelink + (sharelink.indexOf('?') > 0 ? '&' : '?') + 'from_openid=' + localStorage
					.openid + '&hmsr=' + hmsr;
			} else {
	...
	share.qqShare = function(option) {
		...
		if (btn.length > 0) {
			...
			btn.attr({
				target: '_blank',
				href: 'http://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?url=' + url + '&title=' +
					txt + imgHtml + '&summary='
			});
		} //end if
	} //end func
	...
	share.btnShare = function(btn, box) {
		...
		if (shareBtn.length > 0) {
			...
			if (os.weixin) {
				if (shareBox.length == 0) shareBox = $(
					'<aside class="shareBox"><img src="images/common/share.png"></aside>').appendTo(ibase
					.landscapeMode ? 'article' : 'body');
