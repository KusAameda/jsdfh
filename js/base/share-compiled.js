...
var token_id = "bd5d9ebc3281470ea6e73a57604a1049";
//-------------------------------------------------------自定义分享内容
(function () {
	ishare.wxId = 'wx3464c6b9fd10bf5c'; //微信 appid
	...
	if (os.weixin) {
		ishare.from = getQueryString('from');
...
function importShare() {
	...
	share.wxSign = function () {
		...
		function setHttpApi() {
			$.getJSON("http://open.omnimkt.com/api/getWxJsapiSignature/" + token_id, {
				appid: share.wxId,
				url: location.href
			}, function (data) {
				ApiConfig(data);
		...
		function setHttpsApi() {
			$.get("https://open.omnimkt.com/api/getWxJsapiSignature/" + token_id, {
				appid: share.wxId,
				url: location.href
			}, function (data) {
				ApiConfig(data);
		...
		function ApiConfig(data) {
			...
			wx.ready(function () {
				...
				wx.hideMenuItems({
					menuList: ['menuItem:share:appMessage', 'menuItem:share:timeline', 'menuItem:share:qq', 'menuItem:share:weiboApp', 'menuItem:copyUrl', 'menuItem:openWithQQBrowser', 'menuItem:openWithSafari', 'menuItem:share:QZone', 'menuItem:share:email'] // 要隐藏的菜单项，只能隐藏“传播类”和“保护类”按钮，所有menu项见附录3
				});
				share.wxShare();
			}); //end wx.ready
		}
	...
	share.wxShare = function () {
		if (share.wxSigned) {
			...
			var hmsr = getQueryString('hmsr') != null ? getQueryString('hmsr') : 'default';
			if (localStorage.openid) {
				sharelink = sharelink + (sharelink.indexOf('?') > 0 ? '&' : '?') + 'from_openid=' + localStorage.openid + '&hmsr=' + hmsr;
			} else {
				sharelink = sharelink + (sharelink.indexOf('?') > 0 ? '&' : '?') + 'hmsr=' + hmsr;
			}
	...
	share.qqShare = function (option) {
		var url,
		    ...
		    img,
		    imgHtml = '';
		if (option.obj) var btn = option.obj;else var btn = $('a.btnShareQq');
		if (btn.length > 0) {
			url = option.url || window.location.href;
			txt = option.text || "";
			img = option.image;
			...
			btn.attr({
				target: '_blank',
				href: 'http://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?url=' + url + '&title=' + txt + imgHtml + '&summary='
			});
		} //end if
	}; //end func
	...
	share.tbShare = function (url, title, text, image) {
		//重新自定义手淘分享内容
